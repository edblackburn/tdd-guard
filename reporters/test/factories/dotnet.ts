...
function packNuGetPackages(): string {
  ...
  const outputDir = mkdtempSync(join(tmpdir(), 'tddguard-nupkgs-'))
  const solutionDir = join(__dirname, '../../dotnet')

  // TddGuard.Dotnet is the only packable project: Core is an internal
  // implementation detail whose output is embedded into this package.
  const project = 'TddGuard.Dotnet/TddGuard.Dotnet.csproj'

  const result = spawnSync(
    dotnetBinary,
    ['pack', project, '--output', outputDir],
    {
      cwd: solutionDir,
      stdio: 'pipe',
      encoding: 'utf8',
